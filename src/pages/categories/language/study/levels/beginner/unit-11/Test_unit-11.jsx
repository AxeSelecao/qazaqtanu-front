...
function Test_Unit11({ unit }) {
  ...
  const checkAnswer = () => {
    ...
    axios.get(`http://localhost:8000/user/${profileData._id}`).then((res) => {
      if (!res.data.results[0].units[unit].materials[2].completed) {
        handleAddPoints(profileData._id);
        dispatch(addPoint());
        handleMakeComplete({
          unit_name: `unit-${unit + 1}`,
          material_id: res.data.results[0].units[unit].materials[2]._id,
        });
      }
    });
            ...
            return (
              <div className="unit__test-item">
                ...
                <div className="unit__test-answers">
                  {data[0].units[unit].materials[2].items[i].answer_options.map(
                    (option) => {
                      return (
                        <div
