...
function Test_Unit13({ unit }) {
  ...
  let answers = data[0].units[unit].materials[0].answers;
  console.log(answers);
  ...
  const checkAnswer = () => {
    ...
    axios.get(`http://localhost:8000/user/${profileData._id}`).then((res) => {
      if (!res.data.results[0].units[unit].materials[0].completed) {
        handleAddPoints(profileData._id);
        dispatch(addPoint());
        handleMakeComplete({
          unit_name: `unit-${unit + 1}`,
          material_id: res.data.results[0].units[unit].materials[0]._id,
        });
      }
    });
    ...
    <div className="unit">
      <div className="unit__container">
        ...
        <div className="unit__test">
          {data[0].units[unit].materials[0].items.map((item, i) => {
            return (
              <div className="unit__test-item">
                ...
                <div className="unit__test-answers">
                  {data[0].units[unit].materials[0].items[i].answer_options.map(
                    (option) => {
                      return (
                        <div
